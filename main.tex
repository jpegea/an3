...
\usepackage[papersize={156mm,234mm}, margin=0.7cm, includeheadfoot]{geometry}
\usepackage{graphicx}
\usepackage{xcolor}
...
\begin{proof}
  La implicació $(\ref{c_cont:1}) \to (\ref{c_cont:2})$ és
  directa. Per a $(\ref{c_cont:2}) \to (\ref{c_cont:3})$ observem que
  $T$ és contínua en $0$ i és lineal. Per tant, $T(0) = 0$ i existeix
  $r > 0$ tal que $B_E(0,r) \subset T^{-1}(B_F(0,1))$. Per tant, per a
  tot $x \in B_E(0,r)$ tenim que $T(x) \in B_F(0,1)$. Així,
  $\|T(x)\|_F \leq 1$, però
  \[
  ...
  $(\ref{c_cont:3}) \to (\ref{c_cont:4}) \to (\ref{c_cont:5})$ és
  directe i sols queda demostrar
  $(\ref{c_cont:5}) \to (\ref{c_cont:1})$. Que $T(B_E)$ siga fitat
  implica que existeix $C > 0$ amb $\|T(x)\| \leq C$ per a tot
...
\begin{exemple}
  ...
  \]
  Notem qu en els espas $L^p(\Omega)$ la convergència en la norma
